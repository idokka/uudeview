...
char * uuscan_id = "$Id: uuscan.c,v 1.36 2002/04/02 10:04:52 fp Exp $";
...
char *
UUDWrap::ScanHeaderLine (FILE *datei, char *initial)
{
 ...
 */
char *
UUDWrap::ParseValue (char *attribute, int looseValue)
{
...
headers *
UUDWrap::ParseHeader (headers *theheaders, char *line)
{
  char **variable=NULL;
  ...
  if (_FP_strnicmp (line, "From:", 5) == 0) {
    ...
    if ((ptr = _FP_stristr (line, "name")) != NULL) {
      if ((thenew = ParseValue (ptr, !uu_strict_filename))) {
     ...
     */
    if ((ptr = _FP_stristr (line, "name")) != NULL) {
      if (theheaders->fname == NULL && (thenew=ParseValue(ptr, !uu_strict_filename)) != NULL) {
	theheaders->fname = _FP_strdup (thenew);
      }
    }
    variable = NULL;
...
int
UUDWrap::IsKnownHeader (char *line)
{
...
int
UUDWrap::UUScanHeader (FILE *datei, headers *envelope)
{
  char *ptr;
...
int
UUDWrap::ScanData (FILE *datei, char *fname, int *errcode,
  ...
  *errcode = UURET_OK;
  ...
  while (!feof (datei)) {
	...
	p2 = ptr;
	  ...
	  /* Added stuff to permit filenames with no quotes and weird characters */

	  if (uu_strict_filename)
	  {
		  while (isalnum(*p2) || 
			 *p2 == '.' || *p2=='_' || *p2 == '-' ||
			 *p2 == '!' || *p2=='@' || *p2 == '$')
...
fileread *
UUDWrap::ScanPart (FILE *datei, char *fname, int *errcode)
{
