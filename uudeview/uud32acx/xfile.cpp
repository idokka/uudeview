...
// xfile.cpp: Implement self-deleting (X) files
...
BOOL CUud32acxCtrl::XFileDel() 
{
	long le;

if (XFList.IsEmpty())
	return(TRUE);						// OK, just return, nothing there

for (POSITION pos=XFList.GetHeadPosition(); pos != NULL; XFList.GetNext(pos))
	if (!DeleteFile(XFList.GetAt(pos))) {
		le = ::GetLastError();
		if (le != ERROR_FILE_NOT_FOUND && le != ERROR_ACCESS_DENIED)
			return FALSE;					// Something went wrong during delete
		}

XFList.RemoveAll();						// Waste the list
...
STRING CUud32acxCtrl::GetXFile(short row) 
{
	CString strResult;
...
if (row < 0 || row >= XFList.GetCount())		// Not a valid position
	ThrowError(CTL_E_INVALIDPROPERTYARRAYINDEX, IDS_BADFLS);

strResult = XFList.GetAt(XFList.FindIndex(row));	// Get the file
...
void CUud32acxCtrl::SetXFile(short row, LPCTSTR lpszNewValue) 
{
...
if (lpszNewValue == NULL || lpszNewValue[0] == EOS)
	XFList.RemoveAt(XFList.FindIndex(row));		// Delete the entry
else
	XFList.SetAt(XFList.FindIndex(row), lpszNewValue);		// Change the entry

}


//
// Read the X file count
//

short CUud32acxCtrl::GetXFileCount() 
{

if (DesignMode())
	GetNotSupported();					// Runtime only

return(XFList.GetCount());
}

void CUud32acxCtrl::SetXFileCount(short nNewValue) 
{
	SetNotSupported();
}
...
BOOL CUud32acxCtrl::XFileAdd(LPCTSTR File2Add) 
{

	XFList.AddTail(File2Add);				// Append a file

	return TRUE;							// For now, always OK
}



//
// Send a "new step" message to the progress sub
//

void CUud32acxCtrl::NewStep()
{
	...
	FireProgress(UUACT_NEWSTEP, &wrk, 0, 0, 0, 0, &cflag);

	SysFreeString(wrk);
#else
	uuprogress up;

	up.action = UUACT_NEWSTEP;
	up.percent = 0;
...
//
// Read a file onto the clipboard or append the clipboard to a file
//
// Returns True if everything went OK
//
// CFMode allows 3 values; 0 = File->Clipboard; 1 = Clipboard->New file; 2 = Clipboard->Append file
//

BOOL CUud32acxCtrl::ClipboardFile(long hWnd, LPCTSTR Filename, short CFMode) 
{
	MappedFile mf;
	HANDLE memhdl, fh;
	DWORD fs, fsr;
...
switch (CFMode) {
	case 0:					// File to Clipboard
		if (!mf.ReadExisting(Filename) || !::OpenClipboard((HWND) hWnd))
			return(FALSE);		// Couldn't open file or clipboard
	...
	case 2:					// Append clipboard
		if (!::IsClipboardFormatAvailable(CF_TEXT) || !::OpenClipboard((HWND) hWnd))
			return(FALSE);								// No text on clipboard

		if ((memhdl = ::GetClipboardData(CF_TEXT)) == NULL ||	// Couldn't get handle
			 (bp = (byte *) GlobalLock(memhdl)) == NULL) {	// Couldn't lock it
		...
		if ((fh = CreateFile(Filename, GENERIC_WRITE, 0,
...
BOOL CUud32acxCtrl::CRC32(LPCTSTR Filename, long FAR* crc) 
{
	MappedFile mf;

if (!mf.ReadExisting(Filename))
	return(FALSE);						// Couldn't open file
