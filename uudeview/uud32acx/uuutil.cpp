...
void
UUDWrap::UUkillfread (fileread *data)
{
  if (data != NULL) {
...
void
UUDWrap::UUkillfile (uufile *data)
{
  uufile *next;
...
void
UUDWrap::UUkilllist (uulist *data)
{
  uulist *next;
...
void
UUDWrap::UUkillheaders (headers *data)
{
  if (data != NULL) {
    _FP_free (data->from);
    _FP_free (data->subject);
...
int
UUDWrap::UUSMPKnownExt (char *filename)
{
...
int
UUDWrap::UUbhdecomp (char *in, char *out, char *last, int *rpc, 
...
size_t
UUDWrap::UUbhwrite (char *ptr, size_t sel, size_t nel, FILE *file)
{
...
uulist *
UUDWrap::UU_smparts_r (uulist *addit, int pass)
{
