...
int
UUDWrap::UUBrokenByNetscape (char *string)
{
  char *ptr;
  int len;
...
int
UUDWrap::UUNetscapeCollapse (char *string)
{
...
int
UUDWrap::UUValidData (char *ptr, int encoding, int *bhflag)
{
...
int
UUDWrap::UURepairData (FILE *datei, char *line, int encoding, int *bhflag)
{
...
size_t
UUDWrap::UUDecodeLine (char *s, char *d, int method)
{
...
int
UUDWrap::UUDecodeQP (FILE *datain, FILE *dataout, int *state,
...
int
UUDWrap::UUDecodePT (FILE *datain, FILE *dataout, int *state,
	    long maxpos, int method, int flags,
	    char *boundary)
{
...
int
UUDWrap::UUDecodePart (FILE *datain, FILE *dataout, int *state,
	      long maxpos, int method, int flags,
	      char *boundary)
{
  ...
  while (!feof (datain) && *state != DONE && 
	...
	*state = DONE;
	...
	if (tf) {
	  ...
	  if (method == YENC_ENCODED) {
	    if (yepartends)
	      yepartcrc = crc32(yepartcrc, (const unsigned char *) oline, count);
	    yefilecrc = crc32(yefilecrc, (const unsigned char *) oline, count);
	    yepartsize += count;
	  }
	  vlc++; lc[1]++;
	}
...
int
UUDWrap::UUQuickDecode (FILE *datain, FILE *dataout, char *boundary, long maxpos)
{
